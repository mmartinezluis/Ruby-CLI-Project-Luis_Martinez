class QuotesApp::CollaboratingQuote
  ...
  def initialize (quote_hash)
   ...
   # if author_object != nil                                                                  # If a mathcing atuhor object is found,
      quote_hash[:author] = author_object if author_object                                                  # replace the value from the incoming qoute hash's author key (originally a string) by the matching author OBJECT from the Author class;
