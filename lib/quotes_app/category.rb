...
class Category
  ...
  def self.quote_from_category(category_input)                                    # "category_input" is the input number from the user to choose his/her desired caategory from the categories list.
    ...
    if already_exists != nil
      ...
      CollaboratingQuote.print_quote(already_exists)                                    # Print the qoute
    else                                                                          # If the quote is not included in the "@@all" array from the CollaboratingQuote class,
      new_quote = CollaboratingQuote.new(quote_hash)                              # Create a new quote object using the "body" key and the "author" key of the quote hash
      ...
      CollaboratingQuote.print_quote(new_quote)                                   # and print the newly created quote object
    end
  end
end
